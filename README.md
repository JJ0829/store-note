...
| **잠금** | 🟠 **절반만 진짜다.** 매장 PIN(레시피)은 **서버가 막는다** — `STORE_PIN` 을 넣으면 쿠키 없이는 안 그린다. 사장님 PIN(돈·사람)은 **아직 가림막**이고 데이터는 `localStorage` 에 그대로 있다 |
| **실증 매장** | ⛔ 0곳. 기능은 늘었는데 매장에서 검증된 것은 아직 없다 |
| **가격** | ✅ **월 19,000원 / 매장** (직원 수 무관 · [D-012](docs/08_DECISION_LOG.md)). 체크리스트·교육 모드·오늘 할 일·출퇴근은 **무료** |
| **B4 — 점주가 왜 돈을 내는가** | ⛔ **지불 의사는 그대로 0건.** 가격을 정한 것도, 화면을 만든 것도 **점주가 낸다는 근거가 아니다** |
