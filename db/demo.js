...
export function buildDemo(base = BASE_DAY) {
  ...
  const items = [
    ...
    /* ★ 2026-09-18 에 빵 레시피 넷(바게트·크루아상·사워도우·스콘)이 들어오면서
       같이 들어온 둘이다. 단가가 없으면 그 레시피는 원가 화면에서
       「단가를 채우세요」 만 뜨고, 발표에서 그게 뜨면 끝이다. */
    item(demoId("dd", 3), "박력분", 20000, "g", 36_000, "20kg 한 포"),
    item(demoId("dd", 3), "베이킹파우더", 1000, "g", 9_000),
  ];
  ...
  const settings = {
    ...
    /* ★ 「르방」 이 여기 있는 이유는 「기존 르방」 과 같다 — **사는 게 아니라
       우리가 키운 것**이다. 사워도우가 르방 400g 을 쓰는데 그걸 0원으로
       세는 게 아니라 **안 셈**으로 화면에 드러낸다(원가 화면의 `안 셈` 줄). */
    excluded: ["물", "정수", "추출량", "기존 르방", "르방"],
