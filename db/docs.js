...
const 금지문구 = [
  ...
  /* ★ 2026-09-11 추가 — 위 세 개를 적어두고도 못 잡았다.
     `schema_v2.dbml` 에 있던 문장은 "실행해 본 적 없다" 라서 **한 글자 차이로 비켜갔다.**
     같은 뜻을 한국어로 쓰는 방법이 여럿이라, 잡고 싶은 말은 어미까지 다 적어야 한다. */
  ["실행해 본 적 없", "PGlite 에 실제로 올려서 돌린다"],
  ["실행해 본 적이 없", "PGlite 에 실제로 올려서 돌린다"],
  ["돌려본 적 없", "PGlite 에 실제로 올려서 돌린다"],
  ["돌려본 적이 없", "PGlite 에 실제로 올려서 돌린다"],
  ["29개 검사", "검사 건수를 손으로 적지 않는다 — npm run check 가 말한다"],
