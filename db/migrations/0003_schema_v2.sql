-- ===========================================================================
--  0003_schema_v2 — 설계 표 37개를 실제로 올린다
--
--  ★ db/schema_v2.sql 을 그대로 옮긴 것이다. 딱 한 군데만 뺐다 —
--    §10-B(`events`)는 0001_events.sql 로 **이미 올라가 있어서**
--    `create policy` 가 중복으로 걸린다. 표 정의는 schema_v2.sql 이 정본이다.
--
--  ★ 이 마이그레이션은 **앱을 건드리지 않는다.** 표만 만든다.
--    화면은 계속 localStorage 를 쓴다. 옮기는 순서는
--    docs/deliverables/24_DB이관순서.md 에 있다.
--
--  ★ 만들자마자 전부 잠긴다. RLS 정책이 `current_store_id()` 를 보는데
--    로그인이 아직 없어서 늘 null 이다 → **아무도 한 줄도 못 읽는다.**
--    비어 있는 표라 문제가 없고, 열리는 쪽으로 틀리지 않는다.
-- ===========================================================================
