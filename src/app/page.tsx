  ...
  return (
    ...
    <main className="mx-auto min-h-dvh w-full max-w-[560px] bg-zinc-50 px-4 py-8 pb-24 dark:bg-zinc-950 md:max-w-[900px] md:px-6">
      ...
      {/* ★ 태블릿(md 768px+)에서는 두 열 격자 (사장님 요청 2026-09-16).
          ...
          있는 클래스만 태블릿에서 더해진다. **모든 카드가 한 칸씩** 두 개씩
          나란히 선다 — 「할 일」만 두 칸이던 것은 2026-09-18 에 맞췄다. */}
      <div className="md:mt-3 md:grid md:grid-cols-2 md:items-start md:gap-4">
      {/* ---------- 할 일 — 매일 연다. 유일하게 펴 둔다 ----------
          ★ 2026-09-18 — 태블릿에서 이 카드만 두 칸을 차지했다
            (`md:col-span-2`). 사장님 지적: *"왜 혼자 길게 만들었어요"*.
            펴 둔다는 것과 넓게 둔다는 것은 다른 얘기였다. 다른 카드와 폭을 맞춘다.

          ⚠️ 같은 날 프렙 목록이 둘에서 **셋**이 됐다(제빵·바·주기).
            줄이 하나 늘었으므로 **세로로는 조금 길어졌다.** 그건 일부러다 —
            이 카드만 펴 두는 이유가 «매일 여는 것» 이라서고, 목록이 셋이면
            셋을 보여주는 게 맞다. 다만 여기서 더 늘어나면 폰에서 첫 화면이
            스크롤 없이 안 끝난다. **목록을 또 늘릴 때 이 줄을 먼저 읽을 것** —
            발주를 네 번째 목록으로 뺐다가 되돌린 것도 그래서다(탭바와 문이 겹친다). */}
      <Fold
        className="md:mt-0"
        title="할 일"
      ...
      >
        <div className="flex flex-col gap-2">
          <Tile
            href="/roster"
            name="이번 주 근무표"
            /* ★ 출퇴근 화면 맨 위 스위치로도 바로 간다 (2026-09-16).
               사장님 지적: 둘이 따로 있으면 오갈 때마다 홈을 거친다 */
            note="누가 언제 나오는지 정하고, 직원 연락처를 수정합니다 · 출퇴근 화면에서도 바로 갑니다"
          />
          <Tile
            href="/contracts"
            name="근로계약서"
