 ...
 *
 * ★ 2026-09-13 — **이름을 화면의 버튼과 같은 말로 바꿨다.**
 *   전에는 `punch` 한 이름에 `which: "in"|"out"` 이 붙어 있었는데,
 *   화면 버튼은 「출근」「퇴근」이다. 표를 열어 본 사람이 `punch` 가 무엇인지
 *   다시 코드를 뒤져야 했다 — **나중에 실제로 쓸 때 헷갈린다.**
 *   그래서 버튼이 둘이면 이름도 둘로 쪼갠다(출근/퇴근 · 발주_주문함/발주_들어옴).
 *   지표는 사장님이 직접 읽는 표다. 읽는 사람의 말로 적는다.
 */
