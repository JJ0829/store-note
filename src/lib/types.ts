...
export type Trigger =
   ...
   * N일마다 (주기 관리).
   *
   * ★ `null` 이면 **주기를 정하지 않았다** 는 뜻이다 (2026-09-08).
   *
   * 사장님 지적: *"제빙기 청소 1개월마다 체크도 지우지 — 더럽게.
   * 매일 청소하는 곳도 있는데. 6개월마다 이런 거도 쓰는 곳마다 다 달라서
   * 굳이 없어도 될 거 같은데."*
   *
   * **맞는 지적이다.** 제빙기를 매일 닦는 매장에 `1개월마다` 를 띄우면
   * 그 화면은 처음부터 틀린 말을 한다. 법정 항목도 업종·규모에 따라 다르다.
   * **내가 모르는 숫자를 박아놓고 앱이 단정하면, 사장님이 그 숫자를 믿는다.**
   *
   * 그래서 시드는 전부 `null` 로 두고 **매장이 넣게 한다**
   * (`sop:cycleEvery`). 안 넣으면 화면은 기한을 판단하지 않고
   * `마지막 5/11 · 120일 전` 만 보여준다 — 그것만으로도 아무도 기억 못 하던
   * 것을 대신 기억해 준다.
   */
  | { type: "cycle"; everyDays: number | null };
