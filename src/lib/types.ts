 ...
 *   재료 준비 · 홀 정리 · 화장실 청소). 그런데 이것들은 **시작해 놓고 기다리는 일이
...
export type PrepTask = {
  ...
  /** 시작해 놓고 몇 시간 뒤에 쓸 수 있나 (kind: "time") */
   ...
   * 있어도 될 것 같은데"*).
   *
   * **맞는 지적이다.** `지금 시작하면 → 내일 07:43 부터 사용 가능` 이 이미
  ...
  /**
   * **어느 자리에서 하는 일인가** — 화면에서 소제목으로 묶는 이름.
   *
   * ★ `optionOf` · `optional` 과 **또 다른 것**이다. 셋을 헷갈리면 안 된다.
   *   `optionOf`  어느 **카드 안**에 들어가는가 (배치)
   *   `optional`  **진행률 분모**에 들어가는가 (셈)
   *   `group`     **어느 소제목 아래**에 서는가 (자리)
   *
   * 셈에는 아무 영향이 없다. 순서만 바꾼다.
   *
   * 왜 필요했나 — 오후 프렙에 바(bar) 일과 제빵 일이 섞여 있었다.
   * 콜드브루·청·시럽은 바에서, 반죽·르방은 제빵에서 한다. 한 사람이
   * 한 자리에서 쭉 하는데 목록이 섞여 있으면 **오갔다 해야 한다.**
   * (사장님 지적 2026-09-12: "바리스타에서 할 수 있는 일끼리 묶어달라")
   *
   * `null` 이면 소제목 없이 맨 위에 선다.
   */
  group: string | null;
};
