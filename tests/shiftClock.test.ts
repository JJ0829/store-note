...
/* ---------- 정렬: 하루의 순서대로 (일찍 시작한 조가 앞) ----------
   ★ 2026-09-12 뒤집혔다. 전에는 «방금 시작한 조가 앞» 이었다.
   매장 사람은 «오픈 → 미들 → 마감» 을 몸으로 아는데 화면이 그 반대로
   서니 자기 조를 찾는 게 아니라 목록을 다시 읽게 됐다 (사장님 지시). */

test("07:30 에는 제빵이 오픈조보다 앞이다 (05:00 이 먼저 시작)", () => {
...
test("18:00 에는 미들이 마감조보다 앞이다 (11:00 이 먼저 시작)", () => {
...
test("★ 겹치는 조가 새벽에 둘이면 어제 시작한 쪽이 앞이다", () => {
  /* 자정을 넘긴 상태에서 start 를 그대로 쓰면 야간(23:00)이 마감조(14:30)보다
     먼저 시작한 것처럼 보인다. `startedAt` 이 어제치를 음수로 당겨서 푼다. */
  const NIGHT = { name: "야간", start: "23:00", end: "07:00" };
  const on = onDutyNow([NIGHT, CLOSE_LATE], hm(0, 30));
  assert.deepEqual(
    on.map((s) => s.name),
    ["마감조(연장)", "야간"],
    "새벽에 시작 순서가 뒤집혔다",
