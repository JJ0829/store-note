...
| 7 | `presentation/매장수첩.html` — 서버 없이 더블클릭. **오프라인 백업은 이 파일로 못 박는다.** 같은 시연본 사본이 셋 있고 `presentation/app.html`은 head 없는 조각이라 열면 한글이 깨질 수 있다(§6.5.2). ⚠️ 외부 폰트(`fonts.googleapis.com`)를 받으므로 **오프라인에서는 대체 서체로 뜬다** — "인터넷 없이"라고 단정하지 말 것 | `presentation/README.md`, `매장수첩.html:11~13,42~43` |
