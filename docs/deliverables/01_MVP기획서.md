...
| 계산 | 무엇을 대신하는가 | 구현 |
|---|---|---|
| 리드타임 시각 계산 | "지금 시작하면 → 내일 07:43부터 사용 가능 (16시간)" | `PrepView.tsx` `readyAt()` |
...
| 실측 항목 | 값 |
|---|---|
| 사진·영상이 붙을 수 있는 항목 수 | **91개** (포지션 스텝 35 + 레시피 스텝 32 + 프렙 **27** − 묶음 머리 3) (숫자 정본: [21_화면명세.md §1-b](21_화면명세.md)) |
...
|---|---|---|
| 비용 | 무료 | **월 19,000원 / 매장** (직원 수 무관 · [D-012](../08_DECISION_LOG.md))<br>체크리스트·교육 모드·오늘 할 일·출퇴근은 **무료** |
...
**오늘은 아무 일도 안 일어나고 내일 아침에 터진다.** 그래서 신입이 스스로 알 방법이 없고, 어제 뭘 시작해 놨는지 기억에 의존하므로 종이로는 못 잡는다.
...
→ **3축 개념은 유효하다.** seed 19건에 `kind`가 빠짐없이 채워져 있고, 제품 논리(무엇을 왜 미리 시작해 두는가)의 뼈대다. **다만 "화면이 3축으로 갈라진다"고 말하면 사실과 다르다.** 필드 수준 서술과 `kind`/`trigger_type` 중복 문제는 `docs/deliverables/03_ERD.md` 3-8절·11절.
...
|---|---|---|---|---|---|---|
| `p-1` | 내일용 콜드브루 시작하기 | A(time) | **false** | **true** | 16시간 | 매일 14:00 |
...
| `readyAt()` — "지금 시작하면 → 내일 07:43부터 사용 가능". `daysApart()`로 월말(8/31→9/1) 처리 | `PrepView.tsx:63` |
...
### 5.7 부수 결함: "지금 시작하면" 시각이 갱신되지 않는다
...
seed 실측 — 3개 조(제빵·오픈조·마감조)가 쓴다. **미들조는 2026-09-13 에 뺐다.**
...
- **동시 근무조는 카드를 여러 장 그린다.** `active`를 `map`으로 전부 렌더하고 첫 장만 주황색, 나머지는 "같은 시간대"로 표시한다(`NowPanel.tsx:80~`). ~~오픈조와 미들이 실제로 겹친다~~ → **2026-09-13 미들 제거.** 이제 카드는 늘 세 장이고 「근무 중」은 출근을 찍은 조에만 붙는다. 겹칠 때의 정렬은 **시작 시각 내림차순**이라 방금 출근한 조가 위에 온다(`282c0a9`에서 추가).
...
| # | 제약 | 왜 중요한가 |
|---|---|---|
| 1 | **교대 인계가 안 된다** | 오픈조가 태블릿에서 체크한 오후 프렙을 마감조가 다른 기기로 열면 0/5다. **리드타임이 이 제품의 핵심 주장인데 "어제 콜드브루를 시작했나"를 기기 밖에서 확인할 방법이 없다.** 종이를 이긴다고 내세우는 바로 그 지점이 기기 하나에 갇혀 있다 |
...
| 1차 지표 | 질문 횟수의 변화 | 🔬 계획 |
| 2차 지표 | 링크 열람률 / 끝까지 체크한 비율 / `critical` 항목 누락 여부 | 🔬 **수집 경로는 생겼다 (2026-09-10).** `체크` 이벤트가 `critical`·`doneCount`·`total` 을 함께 남기므로 완주율과 `critical` 누락을 셀 수 있다. **다만 "열람률" 의 분모(발송 수)는 여전히 없다** — 링크를 몇 명에게 보냈는지 앱이 모른다 |
...
| # | 이유 | 근거 |
|---|---|---|
| **1** | **baseline이 없다** (⛔ B1) | 코드는 여전히 완료 후 자기보고만 수집한다 (`TrainingMode.tsx` `설문_응답`, `ChecklistView.tsx` `설문_응답`). "줄었다"를 판정할 비교 대상이 없다 |
...
| ~~**3**~~ | ~~**교육 모드 이벤트를 설문과 이을 키가 없다**~~ | ✅ **해소 2026-09-10.** 원인은 `log()` 가 컴포넌트 4개에 복사돼 있었고 그중 교육 모드만 `getSessionId()` 를 안 붙인 것이었다. 함수를 `src/lib/metrics.ts` 하나로 모으고, 설문에 `runId` 를 실었다(끝낼 때 `save(null)` 로 `run` 을 비우므로 `lastRunId` 로 따로 붙든다). **브라우저 실측: `교육_완료` 와 `설문_응답` 가 같은 `runId`·`sessionId` 로 찍힌다.** 복사본이 다시 생기면 `tests/metrics.test.ts` 가 잡는다 |
| **4** | **기기별 저장이라 결과가 돌아오지 않는다** | §6.5 제약 2 |
| ~~**5**~~ | ~~**체크리스트에 체크 이벤트가 없다 + `설문_응답` 가 완주자 한정이다**~~ | ✅ **해소 2026-09-10.** `체크` 이벤트 신설 — `critical`·`doneCount`·`total` 을 함께 남겨서 완주율과 `critical` 누락, 중도 이탈 지점을 셀 수 있다. 설문은 `finished` 대신 **하나라도 체크하면** 뜨고, `finished`·`doneCount`·`total` 을 페이로드에 실어 완주자와 이탈자를 나눈다. ⚠️ **남은 것: "열람률" 의 분모** — `체크리스트_열기` 는 마운트마다 찍히고 링크를 몇 명에게 보냈는지 앱이 모른다. 비율은 여전히 안 나온다 |
...
한 가지 성과는 있다. **`프렙_체크`가 `recoverable` 값을 함께 남긴다**(`PrepView.tsx`). "되돌릴 수 없는 항목을 실제로 체크했는가"를 셀 수 있으므로 **H7의 직접 지표**가 된다. 문서 지표표에 등재되어 있지 않으므로 추가해야 한다.

### 8.6 지표 정의 (갱신본) — 10종

| event | 뜻 | sessionId | 발생 위치 |
|---|---|---|---|
| `체크리스트_열기` | 체크리스트 링크를 열었다 | 있음 | `ChecklistView.tsx` |
| `교육_시작` | 태블릿 교육 시작 (`totalTasks`) | **없음** | `TrainingMode.tsx` |
| `필수항목_확인` | 필수 항목 확인을 눌렀다 (`taskId`) | **없음** | `TrainingMode.tsx` |
| `교육_완료` | 교육 완료 (`durationSec`, `confirmedCount`) | **없음** | `TrainingMode.tsx` |
| `설문_응답` | "선배에게 몇 번 물어봤나" (`askedSenior`) | 화면에 따라 다름 | `TrainingMode.tsx` / `ChecklistView.tsx` |
| `프렙_열기` | 프렙 목록 열람 | 있음 | `PrepView.tsx` |
| `프렙_체크` | 프렙 항목 체크 (**`recoverable` 동반**) | 있음 | `PrepView.tsx` |
| `프렙_배수` | 배수 버튼 사용 (`scale`) | 있음 | `PrepView.tsx` |
| `레시피_열기` | 레시피 열람 | 있음 | `RecipeDetail.tsx` |
| `레시피_배수` | 레시피 배수 사용 | 있음 | `RecipeDetail.tsx` |

⚠️ **표에 없는 것이 더 중요하다: `프렙_체크`에 대응하는 체크리스트 이벤트가 없다.** 프렙 화면은 항목을 체크할 때마다 `프렙_체크`를 남기는데(`PrepView.tsx:168`), 같은 동작을 하는 `/p/[slug]` 체크리스트는 아무것도 남기지 않는다(§8.4 #5). 10종 목록에 `체크`가 없는 이유가 이것이다.
...
| **B4** | **점주의 지불 의사가 미검증** | 사업모델(STEP 5)의 근거 | 🟠 **절반 해소.** ✅ **가격·과금 단위·무료 구간은 정해졌다 — 월 19,000원/매장, 직원 수 무관** ([D-012], 2026-09-17). ⛔ **그러나 점주에게 직접 금액을 물어본 횟수는 여전히 0회다.** 남은 경로 = **점주 5명에게 직접 묻기** |
...
| 칸 | 왜 못 채웠는가 |
|---|---|
| ~~가격 / 과금 단위 / 구독 여부~~ | ✅ **채워졌다 (2026-09-17)** → **월 19,000원 / 매장 · 매장당 월 구독 · 직원 수 무관.** 무료 구간은 체크리스트·교육 모드·오늘 할 일·출퇴근 ([D-012](../08_DECISION_LOG.md)) |
...
| # | 장면 | 근거 |
|---|---|---|
| 1 | **프렙 리드타임 계산** — "지금 시작하면 → 내일 07:43부터 사용 가능", 금요일 발주에 "(주말 배송 없음)" | `/prep/afternoon` |
...
| 3-b | **리드타임과 배수가 한 화면에 같이 보인다** — `p-2`(내일용 반죽) 항목 안에서 "지금 시작하면 → 내일 몇 시" 경고와 "오늘 몇 배?" 환산이 동시에 뜬다. `recipeSlug`로 레시피를 끌어온 것(§5.5). **2026-09-10 에 8건으로 늘었다** — 콜드브루·반죽·르방·청·냉침차·밀크티·시럽·크림폼. 바에서 만드는 부재료에 전부 레시피가 붙었다 | `/prep/afternoon` |
