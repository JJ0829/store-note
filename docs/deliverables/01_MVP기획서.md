...
| **PIN 잠금** | D-008은 LATER → ⚠️ **이 문서가 "배포 전 필수"로 올린다** (§6.6·§10.3) | ⛔ **없음.** src 전체에 인증 코드 0건. `/r` 화면이 사용자에게 "매장 PIN 잠금은 배포 전에 붙입니다"라고 **약속하고 있다**(`src/app/r/page.tsx:38-40`). 현재 보호 수단은 `noindex` 메타뿐 |
...
| **매장 PIN 잠금** | ⛔ **없다.** src 전체에 인증 코드 0건. **D-007 원문에는 이 조건이 없지만 이 문서가 추가한다** — 링크만 알면 레시피(영업비밀)와 `/roster`(개인정보 5종)가 열리고, `/r` 화면이 이미 "배포 전에 붙입니다"라고 약속했다(`src/app/r/page.tsx:38-40`). 자매 문서 두 곳도 배포 전 항목으로 적고 있다(`02` 11절 #11, `03` 10-5절 #1). → §6.3·§10.3 |
...
> **이 표가 배포 전 조치의 정본이다.**
> `02_화면설계서.md` 11절(화면 관점 미구현)과 `03_ERD.md` 10-5절(데이터·개인정보 관점)은 **각자 관점의 항목만 두고 여기를 가리킨다.** 세 문서에 흩어져 있던 목록을 한 곳에 합쳤다.
...
| 2 | `public/media/`의 1×1 PNG 2개 삭제 | **삭제 + 커밋** (추적 파일이다 — §4.4). **히스토리에는 남는다** | 빈 테두리 박스가 떠서 미완성이 더 도드라진다 | §4.4 · `02` 11절 #7 |
| 3 | 시연에 쓸 파일을 `presentation/프렙노트.html` 하나로 못 박기 (`presentation/README.md`에 `public/app.html` 사본도 추가) | 5분 | 셋 중 `presentation/app.html`을 열면 charset 미지정으로 한글이 깨진다 | §6.5.2 |
| 4 | `data/events.jsonl` 백업 (폴더 밖으로 복사) | 1분 | **`.gitignore` 대상이라 날리면 복구 불가**다. 지금 쌓인 58건이 유일한 이벤트 원본이다 | §8.5 |
| 5 | 루트 `README.md`의 낡은 안내 정정 | 30분 | 심사위원이 저장소를 볼 경우 12건이 코드와 어긋난다 | §11 |
...
| 8 | **`public/app.html`을 `public/` 밖으로 빼기** (단일 파일은 USB·메일로 전달) | 5분 | 레시피 전문(원두 도징 g·1:10 비율·식빵 배합)과 근무표 화면이 통째로 공개 URL에 놓인다. **정적 파일이라 `robots: { index: false }`로 덮을 수 없어, 파일을 빼는 것 말고 확실한 수단이 없다** | §6.6 · `02` 11절 #12 · `03` 10-5절 #7 · `03` 8-2절 #8 |
| 9 | **noindex 4곳 추가** — `/`·`/t/[slug]`·`/p/[slug]`·`/prep/[slug]` | 반나절 | 매장 운영 데이터가 검색 노출 대상이 된다 | §6.6 · `02` 2-1절 · `03` 10-5절 #3 |
| 10 | **`robots.txt` 신설** (`/app.html`·`/media/` Disallow 포함) | 30분 | `robots.txt`가 없으면 `/media/촬영목록.md`까지 크롤 대상이다 | §6.6 · `02` 11절 #13 · `03` 10-5절 #2 |
| 11 | **매장 PIN 잠금** (레시피 `/r` 계열 + `/roster`) | ❓ 미산정 | 링크만 알면 영업비밀과 개인정보 5종이 열린다. **화면이 이미 "배포 전에 붙입니다"라고 약속했다** | §6.3 · §6.6 · `02` 11절 #11 · `03` 10-5절 #1 |
| 12 | **메일 본문 `[ 직원 명단 ]`에서 이메일·전화 열 제거** | `roster.ts:136-148` 한 곳 | bcc로 가린 것이 본문에서 다시 드러나고, 개인별 근무·휴무까지 전원이 갖게 된다. 개인정보 지적의 첫 대상이다 | §11.1 · `03` 10-3절 |
| 13 | `og:url` / `NEXT_PUBLIC_SITE_URL`을 실제 주소로 | 5분 | 카카오톡 미리보기가 `localhost`를 가리킨다. **빌드 시점에 박히므로 값을 바꾸면 재빌드해야 한다** | §6.6 |
...
| 자동화 테스트 · CI · 린트 | 0건이고, 배포 전에 만들 근거가 없다. `npm run lint`는 eslint 미설치로 **실행하면 멈춘다** (§6.7) |
