...
| 3 | 시연에 쓸 파일을 `presentation/프렙노트.html` 하나로 못 박기 | ✅ **부분 완료** — `public/app.html`을 삭제해 사본이 3개 → 2개가 됐다(#8). 남은 둘 중 `presentation/app.html`은 charset 미지정이라 한글이 깨질 수 있다 — **시연에는 `프렙노트.html`을 쓴다** | §6.5.2 |
...
| 11 | **매장 PIN 잠금** | 🟠 **성격이 바뀌었다.** 잠금을 만들었지만 **접근 통제가 아니라 가리개다**(브라우저 안에서 검사 + 평문 저장). 그리고 **잠근 곳이 다르다** — 매출·원가·거래처·계약서는 잠겼고 **`/r` 계열과 `/roster`는 안 잠겼다.** 즉 이 항목의 원래 대상은 그대로 남았다 | `06` **V-19 · V-20** |
...
|---|---|---|---|---|
| **15** | **V-12 폰트 — 단일 파일의 Google Fonts `<link>` 3줄 처리** | **발표 전 · 5분** | 발표장 인터넷이 끊기면 글꼴이 시스템 기본으로 떨어져 표가 깨진다. ❓ 사장님 결정(내장 / 제거) | `06` §7-1 #7 · `10` 부록 A #5 |
| 16 | **출퇴근·근로계약 내보내기(백업)** | **배포 전** | **근로기준법 제42조가 3년 보존을 요구하는 정보인데 백업 수단이 0이다.** 브라우저 데이터를 지우면 복구 불가 | `06` §7-1-A #15 = `09` B-1 #12 **(중복이었다)** |
| 17 | **잠금번호 변경·재설정 경로** | 배포 전 | 잊으면 브라우저 데이터 삭제가 유일한 방법이고, 그러면 #16의 데이터가 사라진다. **가용성 결함이 법정 의무 위반으로 이어진다** | `06` §7-1-A #14 = `09` B-1 #13 |
| 18 | **`/roster`를 잠금 대상에 넣을지 결정** | **배포 전 결정** | 매출은 잠기는데 **직원 이름·연락처는 안 잠긴다.** 보호 우선순위가 뒤집혀 있다 (#11과 같은 뿌리) | `06` V-20 · §7-1-A #13 = `09` B-1 #14 |
...
| ~~자동화 테스트~~ | ✅ **생겼다.** `node --test`로 **187개**가 돈다(외부 러너 0개 — 라이선스 감사를 무효화하지 않으려고 Node 내장을 골랐다). `08_테스트자동화.md` 참조 |
