...
| 11 | **매장 PIN 잠금** | 🟠 **성격이 바뀌었다.** 잠금을 만들었지만 **접근 통제가 아니라 가리개다**(브라우저 안에서 검사. **저장값은 평문이 아니라 FNV-1a 계열 단방향 요약값이지만**, 데이터 자체가 localStorage에 그대로 있어 우회는 그대로 된다 — `03_ERD.md` `sop:ownerPin` 행). 그리고 **잠근 곳이 다르다** — 매출·원가·거래처·계약서는 잠겼고 **`/r` 계열과 `/roster`는 안 잠겼다.** 즉 이 항목의 원래 대상은 그대로 남았다 | `06` **V-19 · V-20** |
